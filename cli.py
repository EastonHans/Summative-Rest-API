...
class InventoryCLI:
    ...
    def add_item(self, name, barcode, quantity, price, brand, category="", description=""):
        try:
    ...
    def update_item(self, item_id: int, **kwargs):
        try:
            ...
            response = requests.patch(f"{self.base_url}/inventory/{item_id}", json=payload)
    ...
    def fetch_from_api(self, search_term: str, search_type: str = "barcode"):
        try:
            ...
            payload = {"search_term": search_term, "search_type": search_type}
            response = requests.post(f"{self.base_url}/api/fetch-product", json=payload)
    ...
    def enrich_item(self, item_id: int, search_type: str = "barcode"):
        try:
            ...
            response = requests.post(f"{self.base_url}/api/enrich-item/{item_id}", json=payload)
...
def main():
    ...
    print("Make sure the Flask server is running first (python app.py)")
        ...
        try:
            if choice == '1':
                ...
                if search_type not in ['barcode', 'name']:
                    print("Invalid search type. Use 'barcode' or 'name'")
                    continue
